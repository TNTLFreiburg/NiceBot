    ...
    for adam in [False]: # [True, False]:
        ...
        for subjName in subjects:
            ...
            if model_name in ['lin_reg', 'lin_svr', 'rbf_svr', 'rf_reg']:
                ...
                var_score_train = r2_score(train_set.y.T, train_set_pred)
                corrcoef_train, pval_train = pearsonr(train_set_pred, train_set.y.T[:, 0])
                ...
                if n_seconds_valid_set > 0:
                    ...
                    var_score_valid = r2_score(valid_set.y.T, valid_set_pred)
                    corrcoef_valid, pval_valid = pearsonr(valid_set_pred, valid_set.y.T[:, 0])
                ...
                if n_seconds_test_set > 0:
                    ...
                    plt.ylim(-1, 1)
                    plt.xlim(0,  int(np.round(preds_per_trial.shape[0]/sampling_rate)))
                    plt.savefig(dir_output_data + '/' + subjName +  save_addon_text + '_fig_pred_test.png', bbox_inches='tight', dpi=300)
                    log.info("-----------------------------------------")
