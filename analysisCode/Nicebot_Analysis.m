...
%% Instructions
% This script assumes that your working directory is the root folder of the repository.

% First, load data file of respective subject, e.g.:
subject_experience = 'moderate';
load(fullfile('data', ['allFiles_' subject_experience 'Exp.mat']));
...
% allFiles_*Exp.mat files. Uncomment this section iy you still want to
...
title(['Score and effector distance for subject with ', subject_experience, ' experience'])
xlabel('Time (s)')
...
title(['x/y axis for subject with ', subject_experience, ' experience'])
xlabel('Thumbstick x')
ylabel('Thumbstick y')
ylim([-1, 1])
...
subplot(2,2,3);
plot(r);
title(['correlation - angular velocities vs. score for subject with ', subject_experience, ' experience']);
xlabel('Joints')
...
title(['Score vs effector velocity for subject with ', subject_experience, ' experience'])
...
view(az,el);
%
print(gcf,'figures\view_diagonal_behind','-dpdf','-r1200')

az = -180; el =  0;  % view from left side of subject
view(az,el);
print(gcf,'figures\view_left','-dpdf','-r1200')

az = -90; el =  0.0000;  % view from straight behind side of subject
view(az,el);
print(gcf,'figures\view_straight_behind','-dpdf','-r1200')

az = -90; el =  90;  % topview
view(az,el);
print(gcf,'figures\view_top','-dpdf','-r1200')
clf
...
%% save fig
print(gcf,'figures\score_development_no_exp','-dpdf','-r300');
